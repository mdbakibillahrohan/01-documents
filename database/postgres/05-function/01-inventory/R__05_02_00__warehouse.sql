...
RETURNS varchar(128) AS $change_default_warehouse$
    ...
    BEGIN
        select get_company_by_login_id(p_json->>'created_by') into v_company;
        v_oid := p_json->>'oid';
        select * into v_data from warehouse where oid = p_json->>'oid' and company_oid = v_company->>'oid';

        update warehouse set is_default = 'No' where company_oid = v_company->>'oid';
        update warehouse set is_default = 'Yes', status = 'Active' where oid = v_oid and company_oid = v_company->>'oid';

        v_description := concat( v_data.name ,' Changed warehouse as default by ', v_company->>'login_id');
		insert into activity_log (description, reference_id, reference_name, created_by, company_oid)
		values (v_description, v_oid, 'Warehouse', v_company->>'login_id', v_company->>'oid');
        return v_oid;
    END;
...
-- PGPASSWORD='password' psql -U postgres -d gds -f ./R__05_02_00__warehouse.sql
