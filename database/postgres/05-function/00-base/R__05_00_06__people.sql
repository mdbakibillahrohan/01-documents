...
CREATE OR REPLACE FUNCTION "public".save_people(p_json json) RETURNS varchar(128) AS $save_people$
    ...
    BEGIN
        ...
        values (v_people_oid, p_json->>'name',  NULLIF(p_json->>'mobile_no', ''),
            ...
            v_company_oid, p_json->>'created_by', p_json->>'status');
        select count(1)::int INTO v_total_subledger from subleger where company_oid = v_company->'oid';

        if v_total_subledger = 0 THEN
            SELECT '001' INTO v_next_code;
        else
            select select concat(lpad((max(substring(subledger_code, 8)::int4)+1)::varchar(64), 3, '0')) into v_next_code from subledger where company_oid = v_company->'oid';
        end if;
        ...
        (select ledger_oid from ledger_setting where company_oid = v_company_oid and ledger_key = v_ac_receivable), v_company->'oid');
        ...
        (select ledger_oid from ledger_setting where company_oid = v_company_oid and ledger_key = v_vendor_credit), v_company->'oid');
        ...
        (select ledger_oid from ledger_setting where company_oid = v_company_oid and ledger_key = v_ac_payable), v_company->'oid');
        ...
        (select ledger_oid from ledger_setting where company_oid = v_company_oid and ledger_key = v_credit_note), v_company->'oid');

        IF coalesce((p_json->>'receivable_balance')::float, 0) > 0 THEN
            -- Debit
    	    select JSON_BUILD_OBJECT(
        	    'ledger_oid', (select get_ledger_oid(v_company_oid, v_ac_receivable)),
        	    ...
        	    'subledger_oid', (select get_subledger_oid(v_company_oid, v_people_oid, v_ac_receivable))
    	    ) into v_debit;

    	    -- Credit
    	    select JSON_BUILD_OBJECT(
        	    'ledger_oid', (select get_ledger_oid(v_company_oid, v_adjustment_receivable)),
        ...
        IF coalesce((p_json->>'payable_balance')::float, 0) > 0 THEN
            -- Credit
    	    select JSON_BUILD_OBJECT(
        	    'ledger_oid', (select get_ledger_oid(v_company_oid, v_ac_payable)),
        	    ...
        	    'subledger_oid', (select get_subledger_oid(v_company_oid, v_people_oid, v_ac_payable))
    	    ) into v_debit;

    	    -- Debit
    	    select JSON_BUILD_OBJECT(
        	    'ledger_oid', (select get_ledger_oid(v_company_oid, v_adjustment_payable)),
...
-- select "public".save_people('{ "name": "Tanim", "email": "tanim1109135@gmail.com", "people_type": ["Employee"], "company_oid": "C_DEMO", "created_by": "admin", "designation_oid": "Des-GM","department_oid": "Dept-Fin", "receivable_balance": 500, "payable_balance": 100, "status": "Active" }');
-- select "public".save_people('{ "oid": "RgzLNI", "name": "Rohan", "email": "tanim1109135@gmail.com", "people_type": ["Employee"], "company_oid": "C_DEMO", "created_by": "admin", "designation_oid": "Des-GM","department_oid": "Dept-Fin", "receivable_balance": 500, "payable_balance": 100, "status": "Active" }');
