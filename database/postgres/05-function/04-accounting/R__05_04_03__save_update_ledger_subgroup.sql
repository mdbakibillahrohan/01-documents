...
RETURNS varchar(128) AS $save_update_ledger_setting$
    ...
    BEGIN
        ...
        if length(coalesce(p_json->>'oid', '')) = 0 then
            ...
            values (v_oid, p_json->>'ledger_key', p_json->>'ledger_name', p_json->>'branch_name',
			p_json->>'initial_balance', p_json->>'status', p_json->>'bank_oid', v_company->>'oid', v_company->>'login_id');
        else
            v_action_type := 'update';
            v_oid := p_json->>'oid';
            update ledger_setting set account_no = p_json->>'account_no', account_name = p_json->>'account_name',
			branch_name = p_json->>'branch_name', initial_balance = p_json->>'initial_balance',
			status = p_json->>'status', bank_oid = p_json->>'bank_oid'
