...
RETURNS varchar(128) AS $save_update_ledger_subgroup$
    ...
    BEGIN
        ...
        if length(coalesce(p_json->>'oid', '')) = 0 then 
            ...
            v_oid := p_json->>'oid';
            select ledger_subgroup_code 
            into v_ledger_subgroup_code 
            from ledger_subgroup 
            where oid = v_oid 
            and company_oid = v_company->>'oid';
            update ledger_subgroup set ledger_subgroup_name = p_json->>'ledger_subgroup_name', ledger_group_oid = p_json->>'ledger_group_oid', ledger_subgroup_type = v_ledger_group.ledger_group_type, balance_sheet_item = v_ledger_group.balance_sheet_item, company_oid = v_company->>'oid', edited_by = v_company->>'login_id', edited_on = clock_timestamp()
            where oid = v_oid and company_oid = v_company->>'oid';
        end if;
        -- ledger subgroup save/update by admin
