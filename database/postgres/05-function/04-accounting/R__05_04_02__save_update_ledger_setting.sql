...
RETURNS varchar(128) AS $save_update_ledger_setting$
    ...
    BEGIN
        ...
        select * into v_data from ledger where oid = p_json->>'ledger_oid';
        if length(coalesce(p_json->>'oid', '')) = 0 then
            ...
            values (v_oid, p_json->>'ledger_key', v_data.ledger_name, v_data.ledger_code, v_data.oid, v_company->>'oid');
        else
            v_action_type := 'update';
            v_oid := p_json->>'oid';
            update ledger_setting set ledger_name = v_data.ledger_name,
            ledger_code = v_data.ledger_code, ledger_oid = v_data.oid
        ...
        values (v_description, v_oid, 'Ledger Setting', v_company->>'login_id', v_company->>'oid');
        return v_oid;
    END;
